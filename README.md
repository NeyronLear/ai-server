...
Этот сервер позволяет локально запустить модель Qwen 3 VL и использовать через API, с опциональной поддержкой туннеля CloudPub и базой данных пользователей и историй их чатов. Признаю, что большая часть кода была создана с помощью ИИ, однако дебаггинг кода происходил вручную. Проект был создан в очень сжатые сроки (8 месяцев плотного графика). В любом случае мы гордимся проделанной работой, даже если она не является идеальным примером. Всем, кто случайно наткнется на этот репозиторий – привет!
...
Email и пароль передаются путем указания переменных среды:
для google colab – `email` и `pass`
в остальных случаях – `CLOUDPUB_EMAIL` и `CLOUDPUB_PASS`
...
Options:
  ...
  --db-path NAME       Изменить название базы данных историй чатов
...
Потапов (@NeyronLear) Егор, Черанев (@sasha369) Александр, 2025-2026

Проект стал возможен благодаря:
Google и их сервису Colab
Команде Unsloth (https://github.com/unslothai/unsloth)
Всему сообществу Hugging Face и датасетам прекрасныз людей на этом сайте
Cursor с их ИИ, особенно когда вещи переставали иметь смысл в голове
